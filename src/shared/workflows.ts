...
export const MONEY_MAKING_MATERIAL_RULES_PROMPT =
  '网赚类素材规律：图片单卖点由背景底图、logo/警示语、网赚灵感原子透明图层组成，常见红包、宝箱、金蛋、金币、礼物盒等红黄奖励视觉；生成底图时应让模型少写或不写中文，logo、警示语、大字卖点、按钮短标签等艺术文字优先由代码渲染成文字层后叠图，避免模型生成错字乱码。奖励信息优先用金币、积分、任务权益等非现金表达，不写具体金额。图片大字报要用短卖点、强对比字块和奖励原子建立首秒停留，规则信息优先用代码渲染的图标化短标签或无文字图标表达积分用途、参与路径和规则页入口，避免密集小字、手机界面小字和功能按钮文字导致乱码；图片多卖点是在非网赚起量素材上叠加任务卡片、红包/金币奖励贴片和可信福利提醒，必须避免仿系统通知、伪官方提醒、真实品牌冒用和明显错字。视频单卖点包含大字报滚屏、风景/解压/城市背景、红包掉落/翻动/加载等利益创意；视频多卖点是在老歌等下沉 UGC 上叠加红包、金币特效；真人类通过权威口播、感性口播、多人采访、情景剧建立“可信赚钱”感。合规底线：只表达任务奖励、福利提醒和可参与感，必要时提示规则页查看；禁止保证收益、稳赚、日入、秒到账、虚构提现截图、夸大赚钱效果和误导下载。';

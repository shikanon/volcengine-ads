...
所有工作流在视频生成类节点前必须进入 `video_prompt_optimize` 节点。该节点不生成视频，只产出可直接传给 Seedance / 数字人生成接口的最终视频提示词 artifact；后续 `seedance`、`asset_generator`、`seedance_avatar` 节点必须优先读取该 artifact 发起视频生成。
